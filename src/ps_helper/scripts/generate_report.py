...
def generate_html_report(json_path):
    ...
    def load_scrapy_stats(json_path):
        ...
        if fields_data:
            df_fields = pd.DataFrame({
                ...
                'Complete': [fields_data[k].get('complete', 0) for k in fields_data.keys()],
                'Empty': [fields_data[k].get('empty', 0) for k in fields_data.keys()]
            })
        else:
        ...
        return scrapy_stats, df_status, df_errors, df_timeline, df_fields, resources_formatted, data

    scrapy_stats, df_status, df_errors, df_timeline, df_fields, resources, data = load_scrapy_stats(json_path)
    ...
    fig_bar_status = go.Figure(data=[go.Bar(
        x=df_status['Status'],
        y=df_status['Count'],
    ...
    if df_errors['Count'].sum() > 0 and df_errors.iloc[0]['Error'] != 'Sin errores':
        fig_pie = go.Figure(data=[go.Pie(
            ...
            marker=dict(colors=['#FF5733', '#F8623D', '#C67448', '#838E56', '#00BF71'][:len(df_errors)]),
            textposition='inside',
    ...
    <body>
        <div class="container">
            ...
            <div class="footer">
                Report automatically generated by PS Helper Library v1.0
            </div>
        </div>
    </body>
    </html>
